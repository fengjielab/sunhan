...
OUTPUT = BUNDLE / "02_main_figures" / "v5"
LOGIC_OUT = BUNDLE / "04_logic_and_qa" / "v5"
...
def generate_fig01() -> list[Path]:
    ...
    ax1.text(0.07, 0.96, "Asynchronous human–machine loop determines the operator-experienced intervention", fontsize=17, fontweight="bold", va="top")
    ...
    paths = save(fig, "Fig01_human_machine_fidelity_framework_v5")
    plt.close(fig)
    return paths
...
def generate_fig02() -> list[Path]:
    ...
    paths = save(fig, "Fig02_system_experiment_v5")
    plt.close(fig)
    return paths
    ...
    for item in fig.findobj(match=lambda obj: isinstance(obj, Text)):
        if "Nominal delay implemented with mixed clocks" in item.get_text():
            item.set_text(item.get_text().replace(
                ...
                "Mixed clocks; full C→R replay unavailable",
            ))
            item.set_fontsize(9.6)
    paths = save(fig, "Fig03_realized_intervention_fidelity_v5")
    plt.close(fig)
    return paths
...
def generate_fig04() -> list[Path]:
    ...
    paths = save(fig, "Fig04_EA_outcome_robustness_v5")
    plt.close(fig)
    return paths
...
def main() -> None:
    ...
    report = {
        ...
        "version_boundary": "Only 02_main_figures/v5 and 04_logic_and_qa/v5 are written.",
    }
    LOGIC_OUT.mkdir(parents=True, exist_ok=True)
    (LOGIC_OUT / "v5_figure_qa.json").write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
