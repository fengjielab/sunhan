...
def run_qa(
    ...
    check("full PNG resolution", full_png, "600 dpi", full_png[2] >= 599.0 and full_png[3] >= 599.0)
    ...
    check("90-mm PNG resolution", reduced_info[2:4], "600 dpi", reduced_info[2] >= 599.0 and reduced_info[3] >= 599.0)
...
def main() -> None:
    ...
    full_figure.savefig(png_path, dpi=PNG_DPI, bbox_inches="tight", pad_inches=0.02)
    ...
    reduced_figure.savefig(reduced_png, dpi=PNG_DPI, bbox_inches="tight", pad_inches=0.02)
    plt.close(reduced_figure)

    outputs = [pdf_path, svg_path, png_path, reduced_png]
    report = run_qa(definition, definition_path, outputs[:3], reduced_png, arrow_pairs + reduced_arrows, report_path)
