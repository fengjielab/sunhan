#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""MECH-D-26-00641 confirmatory 2x2 ablation acquisition program.

Formal condition IDs are exclusively ``I``, ``I_H``, ``I_G`` and ``I_H_G``.
All cells use object-conditioned impedance. ``H`` enables object-conditioned
master haptic gain/deadband and ``G`` enables object-conditioned gripper
speed/requested force. Disabled channels use the neutral fixed baseline from
``ablation_design.py``. Formal and pilot metadata are mandatory and schema-v3
logs retain perception, transition, command, phase, saturation and timing data.

Use ``run_scheduled_trial.py`` rather than typing formal metadata manually.
The ``default`` mode remains available for hardware debugging only and must not
be used as a study condition.
"""
...
from experiment_protocol import ExperimentTimeline, PHASE_PREP, json_safe
...
# 默认阻抗
DEFAULT_K_TRANS = FIXED_BASELINE["K_trans"]
DEFAULT_K_ROT = FIXED_BASELINE["K_rot"]
DEFAULT_DAMPING_RATIO = FIXED_BASELINE["damping_ratio"]
DEFAULT_NULLSPACE = 0.5     # 零空间刚度

# 默认力反馈
DEFAULT_K_FB = FIXED_BASELINE["K_fb"]
DEFAULT_DEADBAND = FIXED_BASELINE["deadband"]

# 默认位置映射
DEFAULT_SCALE = FIXED_BASELINE["scale"]

# 夹爪
GRIPPER_SPEED = FIXED_BASELINE["gripper_speed"]
GRIPPER_FORCE = FIXED_BASELINE["gripper_force"]
...
HAPTIC_FORCE_LIMIT_N = 3.0  # 修回实验统一的软件层合力上限；预实验后锁定
# 夹爪控制频率
...
TRAJECTORY_CSV_HEADER = [
    ...
    "mode", "controller_mode", "run_uuid", "run_kind", "session_id", "schedule_id",
    ...
    "vision_class", "vision_locked_class", "vision_label", "vision_confidence",
    "vision_locked",
    "vision_frame_id", "capture_wall_time_ns", "capture_perf_time_ns",
    "frame_enqueue_perf_time_ns", "inference_start_perf_time_ns",
    "inference_end_perf_time_ns", "result_receive_perf_time_ns",
    "vision_queue_age_ms", "parameter_apply_start_perf_time_ns",
    "parameter_apply_end_perf_time_ns", "detection_to_parameter_start_ms",
    "transition_active", "transition_duration_ms",
    "u_h_base_x", "u_h_base_y", "u_h_base_z", "u_g_aperture_N",
    "u_h_cmd_x", "u_h_cmd_y", "u_h_cmd_z", "u_h_cmd_norm_N",
    "haptic_force_limit_N", "haptic_saturated", "effective_force_threshold_N",
    "fusion_delta_K", "fusion_active", "control_target_frequency_hz",
    "control_target_dt_s", "control_dt", "control_compute_time_s",
    "control_overrun_s", "control_deadline_miss",
...
PRESETS = {
    ...
    "medium_obj": {
        ...
        "scale": 3.0, "gripper_speed": 0.05, "gripper_force": 20.0,
    },
    "hard_obj": {
        "name": "🪨 硬物体手感",
...
VISION_LOCK_TIMEOUT_S = 10.0         # 正式/预实验等待有效策略锁定的最长时间
...
def _yolo_process_main(
    ...
    while True:
        ...
        while True:
            try:
                frame_payload = frame_queue.get_nowait()
            except _q.Empty:
                break

        try:
            cycle += 1
            rgb, capture_wall_ns, capture_perf_ns, enqueue_perf_ns, frame_id = frame_payload
            inference_start_perf_ns = time.perf_counter_ns()
            det = mapper.detect_and_map(rgb)
            inference_end_perf_ns = time.perf_counter_ns()
            ...
            if det is not None:
                ...
                det.update({
                    ...
                    "capture_wall_time_ns": int(capture_wall_ns),
                    "capture_perf_time_ns": int(capture_perf_ns),
                    "frame_enqueue_perf_time_ns": int(enqueue_perf_ns),
                    "inference_start_perf_time_ns": int(inference_start_perf_ns),
                    "inference_end_perf_time_ns": int(inference_end_perf_ns),
                })
...
class InteractiveTeleop:
    ...
    def __init__(self, mode: str = "default", record_trajectory: bool = True,
                 ...
                 auto_stop: bool = True, run_kind: str = "debug",
                 session_id: str = "unknown", schedule_id: str = "unknown",
                 trial_order: int = 0, repetition: int = 0,
                 haptic_force_limit: float = HAPTIC_FORCE_LIMIT_N):
        # ── 运行模式 ──
        self.mode = mode
        self._experiment_condition = mode if mode in ABLATION_CONDITIONS else "DEBUG"
        self._condition = ABLATION_CONDITIONS.get(mode)
        self._run_uuid = str(uuid.uuid4())
        self._run_kind = run_kind
        self._session_id = session_id
        self._schedule_id = schedule_id
        self._trial_order = int(trial_order)
        self._repetition = int(repetition)
        self._haptic_force_limit = float(haptic_force_limit)
        ...
        self._experiment_parameters_locked = mode in (
            *ABLATION_CONDITIONS.keys(),
        )

        # ── Vision 模式状态 ──
        self._vision_enabled = mode in ABLATION_CONDITIONS
        self._vision_auto_map = mode in ABLATION_CONDITIONS
        self._vision_stiffness_only = False
        self._vision_force_fusion = False
        ...
        self._vision_locked_class = "unknown"
        self._last_vision_mismatch_frame_id = -1
        ...
        self._vision_capture_wall_ns = 0
        self._vision_capture_perf_ns = 0
        self._vision_enqueue_perf_ns = 0
        self._vision_inference_start_perf_ns = 0
        self._vision_inference_end_perf_ns = 0
        self._vision_result_receive_perf_ns = 0
        self._vision_queue_age_ms = float("nan")
        self._vision_started_perf_ns = 0
        self._vision_timeout_marked = False
        ...
        self._parameter_apply_start_perf_ns = 0
        self._parameter_apply_end_perf_ns = 0
        self._transition_duration_ms = float("nan")
        ...
        self._gripper_measure_busy = False
        ...
        self._control_compute_time = float("nan")
        self._control_overrun = 0.0
        self._control_deadline_miss = False
        self._haptic_base = np.zeros(3)
        self._haptic_aperture_cue = 0.0
        self._haptic_command = np.zeros(3)
        self._haptic_saturated = False
        self._run_config_path = None
        self._applied_condition_params = resolve_parameters(
            mode, PRESETS["vision_medium"]
        ) if mode in ABLATION_CONDITIONS else dict(FIXED_BASELINE)
    ...
    def _smooth_transition(self, target_K_trans: float, target_K_rot: float,
                            target_zeta: float, target_K_fb: float = None,
                            target_deadband: float = None, duration: float = None):
        """Smooth impedance and haptic parameters with one auditable transition."""
        ...
        def _worker():
            try:
                ...
                K_fb_start = self._K_fb_cur
                deadband_start = self._deadband_cur
                K_target = np.array([target_K_trans, target_K_rot])
                zeta_target = target_zeta
                K_fb_target = self._K_fb_cur if target_K_fb is None else target_K_fb
                deadband_target = self._deadband_cur if target_deadband is None else target_deadband

                self._transition_active = True
                self._transition_stop.clear()
                self._parameter_apply_start_perf_ns = time.perf_counter_ns()
                self._timeline.mark(
                    "parameter_transition_start",
                    condition=self._experiment_condition,
                    target_K_trans=float(target_K_trans),
                    target_K_rot=float(target_K_rot),
                    target_zeta=float(target_zeta),
                    target_K_fb=float(K_fb_target),
                    target_deadband=float(deadband_target),
                )
                ...
                for i in range(steps):
                    ...
                    K_fb_mid = K_fb_start + s * (K_fb_target - K_fb_start)
                    deadband_mid = deadband_start + s * (deadband_target - deadband_start)

                    self._K_trans_cur = K_mid[0]
                    self._K_rot_cur = K_mid[1]
                    self._damping_ratio_cur = zeta_mid
                    self._K_fb_cur = K_fb_mid
                    self._deadband_cur = deadband_mid
                ...
                self._K_fb_cur = K_fb_target
                self._deadband_cur = deadband_target
                K_6x6 = self._build_stiffness(self._K_trans_cur, self._K_rot_cur)
                self.ctrl.set_impedance(K_6x6)
                self.ctrl.set_damping_ratio(self._damping_ratio_cur)
            except Exception as e:
                ...
                self._parameter_apply_end_perf_ns = time.perf_counter_ns()
                if self._parameter_apply_start_perf_ns:
                    self._transition_duration_ms = (
                        self._parameter_apply_end_perf_ns - self._parameter_apply_start_perf_ns
                    ) / 1e6
                self._timeline.mark(
                    "parameter_transition_end",
                    condition=self._experiment_condition,
    ...
    def _start_vision_thread(self):
        ...
        self._vision_started_perf_ns = time.perf_counter_ns()
    ...
    def _vision_loop(self):
        ...
        while self._vision_active:
            ...
            try:
                ...
                # ── 送帧 → YOLO 进程 ──
                try:
                    enqueue_perf_ns = time.perf_counter_ns()
                    frame_queue.put_nowait((
                        rgb, capture_wall_ns, capture_perf_ns, enqueue_perf_ns, frame_id
                    ))
                except _q.Full:
                ...
                try:
                    ...
                    # ── 更新共享状态 (线程安全) ──
                    result_receive_perf_ns = time.perf_counter_ns()
                    with self._vision_lock:
                        ...
                        self._vision_frame_id = int(det.get("frame_id", -1))
                        self._vision_capture_wall_ns = int(det.get("capture_wall_time_ns", 0))
                        self._vision_capture_perf_ns = int(det.get("capture_perf_time_ns", 0))
                        self._vision_enqueue_perf_ns = int(det.get("frame_enqueue_perf_time_ns", 0))
                        self._vision_inference_start_perf_ns = int(det.get("inference_start_perf_time_ns", 0))
                        self._vision_inference_end_perf_ns = int(det.get("inference_end_perf_time_ns", 0))
                        self._vision_result_receive_perf_ns = result_receive_perf_ns
                        self._vision_queue_age_ms = (
                            (result_receive_perf_ns - self._vision_capture_perf_ns) / 1e6
                            if self._vision_capture_perf_ns else float("nan")
                        )
                    self._timeline.mark_repeatable(
                        "vision_detection", confidence=self._vision_confidence,
                        detected_class=self._vision_class,
                        frame_id=self._vision_frame_id,
                        queue_age_ms=self._vision_queue_age_ms,
                    )
    ...
    def _refresh_gripper_measurement(self):
        """Schedule a non-blocking gripper-width read outside the 200 Hz loop."""
        if (self.gripper is None or self._gripper_measure_busy or
                self._cmd_busy):
            return
        self._gripper_measure_busy = True
        threading.Thread(
            target=self._execute_gripper_measurement, daemon=True
        ).start()

    def _execute_gripper_measurement(self):
        """Read the gripper without blocking or racing command I/O."""
        try:
            if not self._gripper_io_lock.acquire(blocking=False):
                return
            try:
                state = self.gripper.read_once()
            finally:
                self._gripper_io_lock.release()
    ...
    def run(self):
        ...
        elif self.mode in ABLATION_CONDITIONS:
            self._K_trans_cur = FIXED_BASELINE["K_trans"]
            self._K_rot_cur = FIXED_BASELINE["K_rot"]
            self._damping_ratio_cur = FIXED_BASELINE["damping_ratio"]
            self._K_fb_cur = FIXED_BASELINE["K_fb"]
            self._deadband_cur = FIXED_BASELINE["deadband"]
            self._scale_cur = FIXED_BASELINE["scale"]
            self._gripper_speed_cur = FIXED_BASELINE["gripper_speed"]
            self._gripper_force_cur = FIXED_BASELINE["gripper_force"]
            self.ctrl.set_impedance(self._build_stiffness(
                self._K_trans_cur, self._K_rot_cur
            ))
            self.ctrl.set_damping_ratio(self._damping_ratio_cur)
        ...
        if self._vision_enabled:
            mode_str = (
                f"Confirmatory ablation {self._experiment_condition} — "
                f"adaptive impedance=on, adaptive haptics="
                f"{self._condition.adaptive_haptics}, adaptive gripper="
                f"{self._condition.adaptive_gripper}"
            )
        ...
        try:
            while self.running:
                ...
                if (self._vision_enabled and self._vision_active and
                        not self._vision_locked and not self._vision_timeout_marked and
                        self._vision_started_perf_ns and
                        (time.perf_counter_ns() - self._vision_started_perf_ns) / 1e9
                        > VISION_LOCK_TIMEOUT_S):
                    self._vision_timeout_marked = True
                    self._timeline.mark(
                        "vision_lock_timeout", timeout_s=VISION_LOCK_TIMEOUT_S
                    )
                    if self._run_kind in ("pilot", "formal"):
                        self._timeline.abort("vision_lock_timeout")
                        self.running = False
                        break
                ...
                vision_ready = (not self._vision_enabled) or self._vision_locked
                if (self._timeline.phase == PHASE_PREP and
                        self._timeline.baseline_ready and controller_ready and vision_ready):
                ...
                F_haptic = F_haptic_base.copy()
                F_haptic[2] += grip_force_mag
                self._gripper_force_feedback = grip_force_mag
                self._haptic_aperture_cue = grip_force_mag

                command_norm = float(np.linalg.norm(F_haptic))
                self._haptic_saturated = command_norm > self._haptic_force_limit
                if self._haptic_saturated and command_norm > 0:
                    F_haptic *= self._haptic_force_limit / command_norm
                self._haptic_command = F_haptic.copy()
                ...
                if self._vision_enabled and hasattr(self, '_vision_auto_map') and self._vision_auto_map:
                    with self._vision_lock:
                        ...
                        detection = self._vision_detection
                        detected_class = str(
                            detection.get("class", "unknown")
                        ) if detection is not None else "unknown"
                        detected_frame_id = self._vision_frame_id
                    now_ts = time.time()

                    if profile is not None and (now_ts - det_time) < VISION_DETECTION_HOLD_TIMEOUT:
                        # 有有效检测
                        if not self._vision_locked:
                            detected_label = getattr(profile, "label", "unknown")
                            expected_object = self._timeline.object_id
                            strict_lock = self._run_kind in ("pilot", "formal")
                            lock_eligible = (
                                not strict_lock or
                                (detected_class == expected_object and
                                 detected_label != "unknown")
                            )
                            if (not lock_eligible and
                                    detected_frame_id != self._last_vision_mismatch_frame_id):
                                self._last_vision_mismatch_frame_id = detected_frame_id
                                self._timeline.mark_repeatable(
                                    "vision_object_mismatch",
                                    expected_object=expected_object,
                                    detected_class=detected_class,
                                    semantic_label=detected_label,
                                    frame_id=detected_frame_id,
                                )
                                print(
                                    f"\n  WARNING: expected {expected_object}, "
                                    f"detected {detected_class} ({detected_label}); "
                                    "parameters remain unlocked"
                                )
                            # ── 第一次检测到物体 → 锁定参数 ──
                            preset_key = self._profile_to_preset(profile) if lock_eligible else None
                            self._vision_current_preset = preset_key
                            p = PRESETS.get(preset_key)
                            if p:
                                print(f"\n  👁️ 首次检测到物体 → {p['name']} (参数已锁定)")
                                print(f"     {p['desc']}")
                                applied = resolve_parameters(self._experiment_condition, p)
                                self._applied_condition_params = applied
                                self._smooth_transition(
                                    applied["K_trans"], applied["K_rot"],
                                    applied["damping_ratio"], applied["K_fb"],
                                    applied["deadband"],
                                )
                                self._scale_cur = applied["scale"]
                                self._gripper_speed_cur = applied["gripper_speed"]
                                self._gripper_force_cur = applied["gripper_force"]
                                self._vision_locked_label = detected_label
                                self._vision_locked_class = detected_class
                                self._vision_base_K_trans = applied["K_trans"]
                                self._vision_base_K_rot = applied["K_rot"]
                                self._vision_base_damping = applied["damping_ratio"]
                                self._fusion_delta_K = 0.0
                                self._fusion_active = False
                                if self._vision_force_fusion:
                                    print(
                                ...
                                self._timeline.mark(
                                    "vision_lock", detected_class=self._vision_locked_class,
                                    semantic_label=self._vision_locked_label,
                                    confidence=self._vision_confidence,
                                    condition=self._experiment_condition,
                                    adaptive_haptics=self._condition.adaptive_haptics,
                                    adaptive_gripper=self._condition.adaptive_gripper,
                                    capture_perf_time_ns=self._vision_capture_perf_ns,
                                    result_receive_perf_time_ns=self._vision_result_receive_perf_ns,
                                )
                ...
                # ── 6b. 统一原始数据记录（状态更新完成后） ──
                self._control_compute_time = time.perf_counter() - t_start
                # A deadline is missed only when computation exceeds its budget.
                # Comparing the full period to dt labels normal sleep jitter as a
                # miss and produced the misleading ~100% rate in pilot_v0.
                self._control_overrun = max(0.0, self._control_compute_time - dt)
                self._control_deadline_miss = self._control_compute_time > dt
    ...
    def save_run_config(self):
        """Write the immutable per-run provenance record before hardware starts."""
        path = Path(self._trajectory_dir)
        path.mkdir(parents=True, exist_ok=True)
        config_path = path / f"run_config_{self._run_uuid}.json"
        if config_path.exists():
            raise FileExistsError(f"Run config already exists: {config_path}")
        script_path = Path(__file__).resolve()
        code_paths = [
            script_path,
            script_path.with_name("ablation_design.py"),
            script_path.with_name("experiment_protocol.py"),
            ...
            WORKING_ROOT / "biaoding" / "vision_physics_mapper.py",
        ]
        config = {
            "schema_version": SCHEMA_VERSION,
            "source_baseline_commit": "1c1ad02b07e4d269e09423b0c14879acd482eeab",
            "source_baseline_tag": "MECH-D-26-00641-submitted",
            "run_uuid": self._run_uuid,
            "run_kind": self._run_kind,
            "trajectory_dir": str(Path(self._trajectory_dir).resolve()),
            "condition": self._experiment_condition,
            ...
            "haptic_force_limit_N": self._haptic_force_limit,
            "control": {
                "target_frequency_hz": CTRL_FREQ,
                "target_period_s": 1.0 / CTRL_FREQ,
                "deadline_definition": "control_compute_time_s > target_period_s",
            },
            ...
            "vision_model": {
                ...
                "sha256": (
                    hashlib.sha256(Path(YOLO_MODEL_PATH).read_bytes()).hexdigest()
                    if Path(YOLO_MODEL_PATH).is_file() else None
                ),
        ...
        with config_path.open("x", encoding="utf-8") as stream:
            json.dump(
                json_safe(config), stream,
                ensure_ascii=False, indent=2, allow_nan=False,
            )
        self._run_config_path = str(config_path)
        print(f"Run configuration locked: {config_path}")
    ...
    def _record_trajectory_sample(self, raw_pos, gripper_deg, button, now_perf=None):
        ...
        self._trajectory.append({
            ...
            "trial_order": self._trial_order,
            "repetition": self._repetition,
            "adaptive_impedance": int(bool(self._condition and self._condition.adaptive_impedance)),
            "adaptive_haptics": int(bool(self._condition and self._condition.adaptive_haptics)),
            "adaptive_gripper": int(bool(self._condition and self._condition.adaptive_gripper)),
            ...
            "vision_locked_class": self._vision_locked_class,
            ...
            "vision_frame_id": self._vision_frame_id,
            "capture_wall_time_ns": self._vision_capture_wall_ns,
            "capture_perf_time_ns": self._vision_capture_perf_ns,
            "frame_enqueue_perf_time_ns": self._vision_enqueue_perf_ns,
            "inference_start_perf_time_ns": self._vision_inference_start_perf_ns,
            "inference_end_perf_time_ns": self._vision_inference_end_perf_ns,
            "result_receive_perf_time_ns": self._vision_result_receive_perf_ns,
            "vision_queue_age_ms": self._vision_queue_age_ms,
            "parameter_apply_start_perf_time_ns": self._parameter_apply_start_perf_ns,
            "parameter_apply_end_perf_time_ns": self._parameter_apply_end_perf_ns,
            "detection_to_parameter_start_ms": (
                (self._parameter_apply_start_perf_ns - self._vision_result_receive_perf_ns) / 1e6
                if self._parameter_apply_start_perf_ns and self._vision_result_receive_perf_ns
                else float("nan")
            ),
            "transition_active": int(self._transition_active),
            "transition_duration_ms": self._transition_duration_ms,
            "u_h_base_x": self._haptic_base[0],
            "u_h_base_y": self._haptic_base[1],
            "u_h_base_z": self._haptic_base[2],
            "u_g_aperture_N": self._haptic_aperture_cue,
            "u_h_cmd_x": self._haptic_command[0],
            "u_h_cmd_y": self._haptic_command[1],
            "u_h_cmd_z": self._haptic_command[2],
            "u_h_cmd_norm_N": float(np.linalg.norm(self._haptic_command)),
            "haptic_force_limit_N": self._haptic_force_limit,
            "haptic_saturated": int(self._haptic_saturated),
            "effective_force_threshold_N": (
                self._deadband_cur / self._K_fb_cur
                if self._K_fb_cur > 0 else float("inf")
            ),
            "control_target_frequency_hz": CTRL_FREQ,
            "control_target_dt_s": 1.0 / CTRL_FREQ,
            "control_dt": self._control_dt,
            "control_compute_time_s": self._control_compute_time,
            "control_overrun_s": self._control_overrun,
            "control_deadline_miss": int(self._control_deadline_miss),
    ...
    def _save_trajectory(self, timestamp: str = None):
        ...
        path = Path(self._trajectory_dir)
        path.mkdir(parents=True, exist_ok=True)
        fname = f"{self._experiment_condition}_{self._run_uuid}.csv"
        fpath = path / fname
        ...
        with open(fpath, "x", newline="") as f:
        ...
        events_path = path / f"{self._experiment_condition}_{self._run_uuid}_events.json"
    ...
    def _save_summary(self, timestamp: str):
        ...
        path = Path(self._trajectory_dir)
        path.mkdir(parents=True, exist_ok=True)
        fname = f"{self._experiment_condition}_{self._run_uuid}_summary.json"
        fpath = path / fname
        ...
        mode_info = {
            "mode": self.mode,
            "condition": self._experiment_condition,
            "adaptive_impedance": bool(self._condition and self._condition.adaptive_impedance),
            "adaptive_haptics": bool(self._condition and self._condition.adaptive_haptics),
            "adaptive_gripper": bool(self._condition and self._condition.adaptive_gripper),
        ...
        summary = {
            ...
            "run_uuid": self._run_uuid,
            "run_kind": self._run_kind,
            "run_config": self._run_config_path,
            "session_id": self._session_id,
            "schedule_id": self._schedule_id,
            "subject_id": self._timeline.subject_id,
            "object_id": self._timeline.object_id,
            ...
            "applied_condition_params": self._applied_condition_params,
            "timing": {
                "control_target_frequency_hz": CTRL_FREQ,
                "control_target_dt_s": 1.0 / CTRL_FREQ,
                "parameter_transition_duration_ms": self._transition_duration_ms,
                "vision_queue_age_ms_final": self._vision_queue_age_ms,
                "control_deadline_miss_count": sum(
                    int(row.get("control_deadline_miss", 0)) for row in self._trajectory
                ),
                "haptic_saturation_count": sum(
                    int(row.get("haptic_saturated", 0)) for row in self._trajectory
                ),
            },
...
def main():
    ...
    MODE_CHOICES = list(ABLATION_CONDITIONS) + ["default"]
    parser = argparse.ArgumentParser(description="MECH-D-26-00641 confirmatory ablation teleoperation")
    parser.add_argument("--mode", "-m", type=str, default="I",
                        choices=MODE_CHOICES,
                        help="Confirmatory conditions: I, I_H, I_G, I_H_G")
    parser.add_argument("--load", "-l", type=str, default=None,
                        help="启动时加载参数文件路径")
    parser.add_argument("--no-trajectory", action="store_true",
                        help="关闭轨迹自动录制（默认开启）")
    parser.add_argument("--trajectory-dir", type=str, default=TRAJECTORY_DIR)
    parser.add_argument("--run-kind", choices=["debug", "pilot", "formal"], default="debug")
    parser.add_argument("--session-id", default="unknown")
    parser.add_argument("--schedule-id", default="unknown")
    ...
    parser.add_argument("--trial-order", type=int, default=0)
    parser.add_argument("--repetition", type=int, default=0)
    parser.add_argument("--haptic-force-limit", type=float, default=HAPTIC_FORCE_LIMIT_N)
    ...
    if args.run_kind in ("pilot", "formal"):
        required = {
            "condition": args.mode if args.mode in ABLATION_CONDITIONS else "",
        ...
        missing = [key for key, value in required.items() if value in ("", "unknown")]
        if args.trial_order <= 0:
            missing.append("trial_order")
        if args.repetition <= 0:
            missing.append("repetition")
        if missing:
            parser.error("pilot/formal run is missing locked metadata: " + ", ".join(missing))
        required_directory = "05_pilot_data" if args.run_kind == "pilot" else "06_formal_data"
        if required_directory not in Path(args.trajectory_dir).resolve().parts:
            parser.error(
                f"{args.run_kind} output must be inside a {required_directory} directory"
            )
    if args.haptic_force_limit <= 0:
        parser.error("--haptic-force-limit must be positive")
    ...
    teleop = InteractiveTeleop(
        ...
        haptic_force_limit=args.haptic_force_limit,
    )
