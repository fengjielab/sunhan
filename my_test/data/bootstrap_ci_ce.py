#!/usr/bin/env python3
"""
bootstrap_ci_ce.py — C–E 配对差值的 Bootstrap 95% CI
用于 Mechatronics 版论文统计表

输入: all_trials_135.csv, nasa.md
输出: 完成时间/轨迹/TLX 的 median[IQR] + Bootstrap 95% CI
"""
...
def bootstrap_paired_ci(pairs, n_bootstrap=10000, ci_level=95):
    """配对差值的 Bootstrap 置信区间（基于配对块重抽样）"""
    n = len(pairs)
    diffs = [a - b for a, b in pairs]  # C - E
...
def main():
    ...
    for op in ["P01", "P02", "P03"]:
        ...
        op_diff = mean(op_c) - mean(op_e)
        print(f"    {op}: C={mean(op_c):.2f}s, E={mean(op_e):.2f}s, diff={op_diff:+.3f}s ({abs(op_diff)/mean(op_e)*100:.1f}%)")
    ...
    obs_x, lo_x, hi_x, diffs_x = bootstrap_paired_ci(pairs_tlx)
    print(f"  C–E 均值差: {obs_x:.2f}")
    print(f"  Bootstrap 95% CI: [{lo_x:.2f}, {hi_x:.2f}]")
    ...
    print("=" * 72)
    print()
    print("| Metric | C (median [IQR]) | E (median [IQR]) | Δ (C−E) | Bootstrap 95% CI | Direction |")
    print("|:---|---:|---:|---:|---:|:---|")
    ...
    print(f"| Raw NASA-TLX | {median(c_tlx):.2f} [{np.percentile(c_tlx,25):.2f}, {np.percentile(c_tlx,75):.2f}] | {median(e_tlx):.2f} [{np.percentile(e_tlx,25):.2f}, {np.percentile(e_tlx,75):.2f}] | {obs_x:+.2f} | [{lo_x:.2f}, {hi_x:.2f}] | 3/3 operators ↓ |")
    print()
