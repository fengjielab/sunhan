...
异步人机实验常以固定、自适应、视觉使能或组合模式定义条件，但名义标签不能证明结局窗口内实际经历了相应干预。本文提出两级实际干预保真度约束框架，以 \(N_m\rightarrow C_m\rightarrow R_i\rightarrow Y_i\) 组织名义规范、可执行实现、实际记录干预与结局的证据链。第一阶段通过自动提取或结构化作者审计，把同期规范、存档源码、事件、日志轨迹和采集溯源重建为五字段机器可读状态；第二阶段以确定性规则生成诊断、干预身份、允许的比较层级及措辞边界。在12个预先规定并冻结期望输出的受控工件案例中，两级实现12/12返回预期状态与解释边界。框架随后应用于5名参与者的180次重复遥操作试次，揭示了名义标签无法直接显示的三类断点：G的接触后规范不可恢复，且43/45次在记录接触前激活；F的0.20 s守卫存在时钟域错配，仅3/45次达到接触后至少0.20 s；E的视觉干预呈现39次完全、2次部分和4次零窗口暴露。基于这些证据，纯接触后G效应、正确门控F的增量效应和视觉×力交互等名义析因主张被重新限定为实际配置比较。E–A比较显示，E捆绑配置相对A的接触后0.20–1.00 s操作性超额力冲量平均差为−0.3489 N·s，5/5名参与者方向一致，且全部64种记录选择均保持同向；该结果在 \(n=5\) 下仍属探索性，不构成单一视觉、刚度或力成分的因果证据。该框架把隐藏在条件标签后的实现与暴露差异转化为对实验比较身份的可执行、可审计判定，为异步人机系统中“实际比较了什么”提供可复现的证据基础。
...
本文贡献有三项。第一，提出 `ArtifactEvidence` 与 `EvidenceState` 两级机器可读接口，使“谁依据什么工件赋予状态”可被审计。第二，给出从五字段状态到诊断、身份、比较层级和措辞边界的确定性规则。第三，在一个真实存档人机系统中联合重建规范、源码、时钟、事件、指令轨迹、窗口暴露及180条精确采集身份，并区分出规范缺失、时钟域错配和窗口暴露异质三类性质不同的断点，据此重新确定可支持的实际配置比较。
...
本文建立了从规范、源码、事件、日志和溯源到五字段证据状态，再由确定性规则生成诊断、比较身份与科学措辞边界的两级接口；12/12个预先规定并冻结期望输出的受控工件案例均返回预期状态与解释边界。在180次真实存档试次中，框架进一步区分出G的接触后规范不可恢复、F的时钟域错配和E的窗口暴露异质，并据此把纯接触后力效应、正确门控增量效应和视觉×力交互等名义析因主张重新限定为证据支持的实际配置比较。E和F仍是多参数捆绑配置，E–A的方向一致探索性结果不构成单一成分因果效应；同样，保真度通过只保留干预身份，不单独授权因果解释。实际干预保真度框架由此把隐藏在名义标签后的实现与暴露差异转化为可执行、可审计的比较身份判定，为闭环人机系统中“实际比较了什么”提供可复现的证据基础。
