...
Fixed teleoperation settings are poorly suited to objects with different fragility, geometries, and grasping requirements. This paper presents an object-conditioned framework for coordinated parameter reconfiguration across multiple channels of a haptic teleoperation system. Object semantics are mapped through an operation-oriented strategy layer to a joint parameter vector that includes slave-side Cartesian impedance, operator-facing haptic-interface settings, and gripper execution parameters. To integrate perception and control operating at different rates, a single-slot frame queue, a two-slot result queue, and non-blocking polling decouple 15-fps visual perception from the nominal 200-Hz supervisory teleoperation loop. The first valid mapped result triggers a one-shot target-parameter update, after which strategy locking prevents repeated switching within the trial. Three operators completed 135 grasp-and-transfer trials involving six objects and five control modes. The full cross-channel mode achieved the shortest median completion time (19.57 s, IQR [18.41, 20.05]), the highest observed success rate (26/27, 96.3%), and the lowest Raw NASA-TLX score (48.67, IQR [47.67, 51.83]). Compared with impedance-only scheduling, the mean completion time was reduced by 1.79 s (matched-block bootstrap 95% CI [1.10, 2.51] s), corresponding to an 8.5% reduction. The same direction of improvement was observed for all three operators and all six objects. Within the present three-operator experiment, coordinating the haptic interface and gripper with slave-side impedance produced better task-level performance than impedance-only scheduling, although the individual contributions of the added channels remain unresolved.
...
A single set of control parameters is unlikely to suit fragile objects, lightweight containers, and rigid tools equally well. Lower stiffness and gripping force can promote gentle contact but may compromise positioning and transfer stability. Higher stiffness and gripping force may accelerate manipulation but can increase the risk of impact or deformation. To create distinguishable task demands in compliance, haptic cues, and gripper execution, the benchmark set used in this study comprised an apple, a banana, a paper cup, a bottle, a computer mouse, and a pair of scissors. This controlled set enabled evaluation of object-conditioned cross-channel parameter reconfiguration across heterogeneous grasping tasks.
...
Visual information has also been used to select or adapt robot impedance. A vision-and-voice tele-impedance method selected slave-side impedance according to object properties while retaining an operator confirmation or correction step [10]. Visual-impedance control has combined visual and force information in a visual-feature space [11]. Siegemund et al. derived stiffness from object geometry, material, and environmental relations [13], whereas Jekel et al. generated task-relevant stiffness matrices from visual, gaze, and language inputs [14]. These studies share the objective of converting visual task information into slave-side stiffness or impedance settings.

Beyond impedance adaptation, teleoperation research has used intention inference, automatic switching, and shared control to reduce operator decision-making demands [12,15,16,24]. Haptic feedback has also been used to improve manipulation under restricted vision or across objects with different stiffness properties [17,22,25]. Related studies have addressed contact assembly, human and environmental stiffness estimation, rotational impedance, scaling transitions, and multi-stiffness interaction interfaces [18–21,23]. These approaches alter control authority, slave-side impedance, or feedback presentation, but comparatively little attention has been given to using object semantics as a common task condition for jointly configuring the slave-side mechanical response, the operator-facing haptic interface, and gripper execution. The central question of this study is therefore not whether vision can be used to tune impedance, but whether object semantics can drive coordinated reconfiguration across these three mechatronic subsystems and whether the complete configuration provides task-level benefits beyond the visual-semantic display, manual selection, and impedance-only scheduling.

We propose an object-semantic-driven framework for cross-channel mechatronic parameter reconfiguration in haptic teleoperation of heterogeneous objects. A three-level mapping from object semantics to an operation-oriented strategy and then to a joint parameter vector converts task information into coordinated settings for slave-side impedance, the master-side haptic interface, and gripper execution. After task initiation, the first valid mapped result triggers a single target-parameter update. The selected strategy is then locked for the remainder of the trial to prevent repeated switching caused by subsequent detection fluctuations. Five experimental modes—fixed parameters, operator selection, full cross-channel reconfiguration, a visual-semantic cue with fixed parameters, and impedance-only scheduling—were designed to examine the roles of the individual workflow components. The main contributions are as follows:

1. **Cross-channel coordination of mechatronic parameters.** Object-dependent configuration is extended from slave-side impedance alone to three coupled subsystems: the slave-side mechanical response, the operator-facing haptic interface, and gripper execution. The resulting parameter vector addresses the complete teleoperation chain rather than a single control channel.
...
The online stage uses lightweight discrete parameter reconfiguration rather than post-contact continuous optimization or persistent visual servoing. The main contribution is not a single parameter lookup but the coordinated relationship established among object semantics, slave-side impedance, the master-side haptic interface, and gripper execution, together with an asynchronous event-triggered implementation and strategy locking suitable for a human-in-the-loop teleoperation system. The experiments further examine whether this cross-channel configuration yields task-level benefits that are not reproduced by impedance-only scheduling.
...
**Fig. 2(b).** Illustrative timing of the asynchronous perception–control implementation. Color frames are acquired at 15 fps and passed through a single-slot frame queue to an independent YOLO11n process; a controlled visual test yielded a mean wall-clock processing time of 48.19 ms per image. Class-confidence results are returned through a two-slot result queue. The nominal 5-ms supervisory update polls this queue without blocking. The first result that reaches the confidence threshold is converted through the class-to-strategy mapping and creates a one-shot strategy event. Modes C and E update their enabled parameters, whereas mode D retains fixed parameters and presents the semantic result. Translational stiffness, rotational stiffness, and damping ratio follow an approximately 300-ms smoothstep transition, while the enabled haptic-interface and gripper settings change at the event. Subsequent detections do not change the strategy within the same trial. A low-confidence result or absence of a detection leaves the initialized state unchanged; a detected class without an explicit mapping is assigned to the balanced default strategy. The timing diagram is illustrative because the formal trials did not contain a complete per-trial visual-event log or synchronized timestamps for the strategy event and physical contact.

RGB frames are acquired every 66.7 ms and written to a frame queue with a capacity of one, thereby preventing stale images from accumulating. YOLO11n runs in an independent process and returns class-confidence results through a queue with a capacity of two. The nominal 200-Hz supervisory loop polls this queue every 5 ms without waiting for inference. Camera acquisition, perception, and operator motion therefore proceed in parallel. The measured value of 48.19 ms is the mean wall-clock processing time per image, not the camera sampling period. The formal trial records contain task outcomes and active parameter states, but they do not permit reconstruction of every class result, confidence value, trigger, or fallback event.
...
The controller internally generated the damping matrix from the active diagonal stiffness matrix and scalar damping ratio. For the implementation used here,

\[
\mathbf{D}(c)=2\zeta(c)\sqrt{\mathbf{K}(c)},
\]

where the square root is applied element by element to the diagonal entries. No Cartesian effective-mass or inertia matrix was identified or specified, so this expression is reported as the controller implementation rather than as a mass-normalized critical-damping model.
...
where $K_f$ is a dimensionless force-scaling factor and $d$ is a per-axis dead-zone threshold in newtons. The software did not apply an additional saturation to this base force-feedback command; output protection was left to the Omega.7 driver and hardware.
...
With $G_{\mathrm{cue}}=0.3$ and $F_{\mathrm{cue,max}}=1.0$ N, the implemented cue was $u_g=0.3\alpha_g$ N and therefore remained within $[0,0.3]$ N. It was updated in every nominal 200-Hz supervisory cycle. A more open Omega.7 gripper produced a larger cue; the signal did not use Franka Hand aperture, master–slave aperture error, or button state. It is therefore a master-gripper-aperture cue rather than remote gripper or grasp-force feedback.
...
The three parameter channels act on different elements of the teleoperation system. Panda impedance defines the slave-side response to pose error and disturbance, the Omega.7 settings determine how estimated external force is presented to the operator, and the Franka Hand settings determine grasp execution. The proposed method coordinates these settings from a common task condition. It is a task-dependent cross-channel reconfiguration, not a dynamic master–slave impedance-matching law. Haptic transparency and closed-loop fingertip-force accuracy were not evaluated.
...
When the supervisory loop read the first threshold-qualified result, the parameters enabled in the active mode were updated once and then locked for the remainder of the trial. Translational stiffness, rotational stiffness, and damping ratio followed a smoothstep interpolation over approximately $T_s=0.30$ s. With $\tau=\operatorname{clip}((t-t_0)/T_s,0,1)$ and $s(\tau)=3\tau^2-2\tau^3$, each interpolated quantity $q\in\{K_t,K_r,\zeta\}$ was updated as

\[
q(t)=q_0+s(\tau)(q_\mathrm{target}-q_0).
\]

The controller recomputed $\mathbf{D}$ from the current $\mathbf{K}$ and $\zeta$ during this transition. Enabled values of $K_f$, $d$, $v_g$, and $F_g$ changed immediately at the strategy event. Because operator motion was not gated by perception and the strategy event was not synchronized with physical contact in the formal logs, the mechanism is described as early-task one-shot reconfiguration; pre-contact completion is not claimed for every trial.
...
The camera window could be closed by pressing `q`, and window visibility was not logged during the formal experiment. The interface descriptions in Table 3 therefore refer to the default software behavior rather than verified continuous visibility in every trial.

The parameter values were selected as discrete engineering operating points. Two researchers screened candidate combinations on representative objects before the formal experiment and excluded settings associated with sustained oscillation, clear haptic discomfort, grasp failure, or visible object damage. The final values were fixed before the 135 formal trials. The low fragility-priority point used 50 N/m translational stiffness, 8 N requested grasp force, and 0.02 m/s gripper speed to reduce contact and closing severity. The balanced point used 150 N/m, 15 N, and 0.05 m/s to avoid both excessive deformation and unreliable holding. The stability-priority point used 200 N/m, 20 N, and 0.10 m/s to favor pose retention and transfer stability. These values are working points for the present setup; they were not derived from material-strength tests and do not establish a universal damage-free envelope.
...
| $\zeta$ | Lower damping | Stronger damping | Avoid sustained oscillation and excessive sluggishness | 0.8 / 1.0 / 1.2 |
...
4. Use the first threshold-qualified strategy to update the parameter channels enabled by the current mode. Interpolate $K_t$, $K_r$, and $\zeta$ over approximately 300 ms and update the enabled haptic-interface and gripper settings immediately.
...
A misclassification as another known class invoked the strategy assigned to that class. A detected label without an explicit mapping invoked the balanced default strategy, whereas a missing or low-confidence detection caused no update. All resulting settings remained within the three predefined operating points, but bounded parameters do not guarantee that the selected strategy is appropriate for the actual object. The formal trial data did not contain a complete visual-event history and therefore cannot establish how often misclassification or the default branch occurred during the main experiment.

Robot collision detection, Franka Hand force settings, zero-force commands on exit, a standardized initial pose, device-level Omega.7 protection, and a manual emergency stop supported experimental operation. The added master-gripper-aperture cue was limited by the implemented mapping to 0.3 N, but the base haptic command did not have an additional software saturation. These measures are operating safeguards, not a safety certification for perception or parameter-selection errors.
...
The independent YOLO process, bounded queues, and non-blocking polling removed visual inference from the synchronous call path of the nominal 200-Hz supervisory update. The median logged cycle time was approximately 5.07 ms, although the distribution had a long tail. The implementation therefore demonstrates asynchronous integration but not hard real-time performance. The main trial logs did not contain a complete sequence of visual events or synchronized timestamps for the strategy event and physical contact. They consequently cannot establish per-trial trigger correctness, fallback frequency, or whether every update was completed before contact.
...
The C–E comparison is the primary system-level contrast in this study. Both modes used the same class-to-strategy mapping, the same impedance settings, and the same visual-semantic interface by default. Mode C additionally updated the master-side haptic gain, force deadband, gripper speed, and requested grasp-force setting. This design removes the two additional channel groups simultaneously rather than ablating individual parameters. It can therefore assess the overall difference between the complete parameter set and impedance-only scheduling, but it cannot identify the contribution of each added channel.
...
The median master-side trajectory lengths were 0.697 m in mode C and 0.732 m in mode E. The mean paired difference was 0.024 m, with a bootstrap 95% CI of [−0.014, 0.059] m. Because this interval crossed zero, the completion-time advantage of mode C cannot be attributed simply to a shorter geometric path. Pause counts showed an overall direction favoring mode C, suggesting that the benefit of full cross-channel configuration may be reflected in movement continuity rather than path geometry. Because no separate correction or regrasp measure was available, a more specific behavioral mechanism cannot be assigned.
...
Pause count was calculated from the master-side trajectory sampled at approximately 200 Hz. A pause was defined as a continuous interval during which master-device speed remained below 0.005 m/s for at least 0.30 s, thereby excluding momentary decelerations and identifying more sustained interruptions of movement. Mode C had a median pause count of 3 [2, 3.5] and a mean of 2.74 ± 1.23. Mode E had a median of 3 [2, 5] and a mean of 3.41 ± 1.67. Although the two modes had the same median, mode C had a lower mean pause count in each of the three strategy classes, consistent with the direction of its completion-time advantage. This result is treated only as a descriptive indication that full cross-channel reconfiguration may reduce interruptions and is not used to establish a specific process mechanism independently.
...
Mode C had one failure in 27 trials, the lowest observed failure count among the five modes. This pattern indicates favorable task-level robustness under the present experimental conditions. Because the experiment evaluated the combined configuration and did not isolate causal effects of slave-side impedance, the master-side haptic interface, and gripper execution, the failure results cannot be attributed to any single parameter channel.
...
The study involved three operators and repeated measurements from the same participants. All operators were right-handed but used the left-positioned Omega.7 with the left hand. Only translational master motion was mapped, while the end-effector orientation remained fixed. Objects were manually reset within a prescribed region and approximate orientation, so small placement differences were present. The nine preassigned mode sequences reduced reliance on one fixed order but did not provide complete positional balance. The results therefore apply to the present device layout, training conditions, motion mapping, object placement protocol, and participant sample.

The 180-image test used independent photographs but controlled object instances, viewpoint, background, and lighting. It establishes performance for that image set, not per-trial recognition or trigger correctness in the 135 teleoperation trials. The main logs did not preserve a complete visual-event history, the frequency of the balanced-default branch, or continuous interface visibility. Misclassification to another known class and use of the default strategy therefore cannot be excluded. The class-to-strategy mapping is manual: a new label can be added without controller retraining only when the detector already recognizes it reliably; otherwise, a new or retrained detector is required. The system does not infer unseen physical properties or an appropriate strategy automatically.

The parameter sets were engineering working points chosen through qualitative screening rather than optimization or material-strength testing. The C–E comparison removes the haptic-interface and gripper groups together and cannot separate their contributions. The base Omega.7 force-feedback command had no additional software saturation, and the Franka Hand `force` value was a requested grasp setting rather than a measured fingertip force. These implementation choices should be considered when transferring the parameter values to another device.

Success and failure were judged in real time by one designated researcher using predefined criteria. Trials were not systematically video-recorded or independently re-rated, so slip and visible-damage labels retain an observational component. A failure did not end timing immediately, and a dropped object could not be regrasped; the operator instead completed the common terminal procedure. This avoids artificially short failure times but does not provide stage-specific failure durations. The available records also lack synchronized contact time, separate selection time in mode B, regrasp events, quantitative contact quality, and a direct measure of corrective action. Pause count is therefore an exploratory continuity measure only.

Visual inference was removed from the synchronous call path of the nominal 200-Hz update, but the cycle-time logs retained a long tail, so hard real-time performance is not claimed. Future work should add synchronized perception, contact, interface, and mode-selection logs; use channel-specific ablations; record video for independent outcome assessment; include active orientation control and less constrained object placement; and recruit a larger and more diverse participant sample.

## 6 Conclusion

This paper presented an object-conditioned method for cross-channel mechatronic parameter reconfiguration in haptic teleoperation of heterogeneous objects. A three-level mapping from object semantics to an operational strategy and then to a joint parameter set coordinated slave-side impedance, the master-side haptic interface, and gripper execution. An asynchronous one-shot update mechanism integrated the configuration process into the teleoperation system while keeping low-rate perception outside the synchronous supervisory path.
