...
A single set of control parameters is unlikely to suit fragile objects, lightweight containers, and rigid tools equally well. Lower stiffness and gripping force can promote gentle contact but may compromise positioning and transfer stability. Higher stiffness and gripping force may accelerate manipulation but can increase the risk of impact or deformation. To create distinguishable task demands in compliance, haptic cues, and gripper execution, the benchmark set used in this study comprised an apple, a banana, a paper cup, a bottle, a computer mouse, and a pair of scissors. This controlled set enabled evaluation of object-conditioned coordinated parameter reconfiguration across heterogeneous grasping tasks.
...
The online stage uses lightweight discrete parameter reconfiguration rather than post-contact continuous optimization or persistent visual servoing. The main contribution is not a single parameter lookup but a strategy-level relationship among object semantics, slave-side impedance, the master-side haptic interface, and gripper execution, together with an asynchronous event-triggered implementation and strategy locking suitable for a human-in-the-loop teleoperation system. The experiments further examine whether the complete coordinated configuration yields task-level benefits that are not reproduced by impedance-only scheduling. In mechatronic terms, the work treats perception, control, haptics, and grasp execution as an integrated reconfiguration problem rather than as independent modules.
...
where the square root is applied element by element to the diagonal entries. In this paper, $\zeta$ is treated as a dimensionless damping-scaling parameter used by the controller implementation. It is not interpreted as a physically identified Cartesian critical-damping ratio because no effective Cartesian mass or inertia matrix was identified or specified.
...
where $K_f$ is a dimensionless force-scaling factor and $d$ is a per-axis dead-zone threshold in newtons. The software did not apply an additional saturation to this base force-feedback command. The Omega.7 was operated through its standard driver and hardware configuration, but saturation status was not logged and no specific driver-level command clamp is claimed here.
...
The median master-side trajectory lengths were 0.697 m in mode C and 0.732 m in mode E. The mean paired difference was 0.024 m, with a bootstrap 95% CI of [−0.014, 0.059] m. Because this interval crossed zero, the completion-time advantage of mode C cannot be attributed simply to a shorter geometric path. Pause counts were lower on average in mode C but had the same median in modes C and E. This exploratory pattern does not establish movement continuity or any other behavioral mechanism, particularly because no separate correction or regrasp measure was available.
...
Pause count used an engineering threshold of 0.005 m/s for at least 0.30 s. No threshold-sensitivity analysis or prespecified inferential test was performed, so the pause result remains exploratory. Visual inference was removed from the synchronous call path of the nominal 200-Hz update, but the cycle-time logs retained a long tail; hard real-time performance is therefore not claimed. Future work should add synchronized perception, contact, interface, and mode-selection logs; perform pause-threshold sensitivity analysis; record video for independent outcome assessment; include active orientation control and less constrained object placement; and recruit a larger and more diverse participant sample.
