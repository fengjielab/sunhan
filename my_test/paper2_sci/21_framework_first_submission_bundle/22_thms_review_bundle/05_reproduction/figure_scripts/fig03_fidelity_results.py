...
def plot_g_panel(ax: plt.Axes, source: pd.DataFrame) -> None:
    ...
    annotation = (
        ...
        "Median contact-to-\n"
        "activation: −1.214 s\n"
        "No independent archived\n"
        "post-contact G specification;\n"
        "the label cannot support a\n"
        "post-contact effect claim."
    )
    ax.text(
        0.18,
        1.0,
...
def plot_f_panel(ax: plt.Axes, source: pd.DataFrame) -> None:
    ...
    ax.set_title("F: nominal gate versus realized activation", loc="left", pad=7)
    ...
    late = f.loc[f["activation_relative_contact_s"].gt(0.30)]
    inset = ax.inset_axes([0.57, 0.30, 0.39, 0.34])
    inset.axvline(0.20, color=MODE_COLORS["F"], linewidth=0.75, linestyle=(0, (3.0, 2.0)))
    inset.scatter(
        late["activation_relative_contact_s"],
        late["F_trial_rank"].astype(int),
        s=17,
        marker="D",
        facecolor=MODE_COLORS["F"],
        edgecolor="white",
    ...
    inset.set_xlim(0.9, 2.85)
    inset.set_ylim(45.6, 41.8)
    inset.set_xticks([1.0, 2.0, 2.8])
    inset.set_yticks([43, 44, 45])
    inset.tick_params(labelsize=5.7, length=2.0)
    inset.set_title("3 late trials (full values)", fontsize=5.8, pad=2.0)
    inset.spines["top"].set_visible(True)
    inset.spines["right"].set_visible(True)
    inset.spines["top"].set_linewidth(0.55)
    inset.spines["right"].set_linewidth(0.55)
