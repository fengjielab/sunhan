...
def main() -> None:
    ...
    parser.add_argument("--output-stem", default="outputs/Fig7_a_operator_consistency_final")
    args = parser.parse_args()

    pivot = load_paired_data(resolve_trial_file(args.trial_file))
    fig, ax = plt.subplots(figsize=(7.0, 3.0))
    draw_panel_a(ax, pivot, "(a)")
    fig.subplots_adjust(left=0.12, right=0.985, bottom=0.15, top=0.96)
    save_figure(fig, Path(args.output_stem))
    plt.close(fig)
