...
- `03_outcome_registry/`: Supplementary Tables S1--S6, including outcomes, mode sequences, evidence boundaries, parameter design, vision validation, and object-by-mode completion times.
- `04_nasa_tlx/`: raw NASA-TLX records and summaries. `nasa.md` is comma-separated data despite its historical extension.
- `05_vision_validation/`: frozen 180-image closed-set vision-validation inputs, results, and images. The mean per-image inference time in `vision_validation/results/vision_validation_per_image.csv` is 48.192906 ms.
...
- `09_current_manuscript_snapshot/`: current submission snapshot, including LaTeX source, bibliography/style files, current raster figures (`fig1.jpg`, `fig2.png`--`fig9.png`), and compiled PDFs.

`DATA_DICTIONARY.md` defines the data fields and `FIGURE_SOURCE_MAP.md` maps the revised manuscript figures (including Supplementary Fig. S1) to the available source material.
