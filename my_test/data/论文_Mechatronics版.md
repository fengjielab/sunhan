...
**Control-loop profiling.** To verify the real-time behavior under realistic conditions, we performed a dedicated control-loop profiling experiment on the same Omega.7–Panda platform. The operator moved the master device with natural speed variations while the 200 Hz control loop executed all standard modules (Omega.7 read, incremental mapping, Cartesian impedance, force-feedback rendering, and gripper control). Two conditions were tested: (i) default mode without visual inference, and (ii) vision mode with the YOLO11n sub-process running asynchronously (~50 ms/frame). In each condition, per-cycle timestamps were recorded via `time.perf_counter()` for approximately 2 minutes. Table 1b summarizes the statistics.

**Table 1b.** Control-loop period statistics from real-hardware profiling (approx. 2 min each).

| Metric | Default (no vision) | Vision (YOLO async) |
|:---|---:|---:|
| Nominal period | 5.00 ms | 5.00 ms |
| **Median period** | **5.14 ms** | **5.07 ms** |
| Mean period | 7.81 ms | 7.84 ms |
| Standard deviation | 11.92 ms | 12.22 ms |
| Maximum period | 100.47 ms | 185.43 ms |
| Cycles > 10 ms | 1,656 / 25,208 (6.57%) | 1,273 / 19,267 (6.61%) |
| Total cycles | 25,208 | 19,267 |

The median period (5.14 ms and 5.07 ms) closely matched the nominal 5.00 ms target in both conditions. The fraction of cycles exceeding 10 ms remained nearly identical (~6.6%) regardless of whether the YOLO sub-process was active, confirming that the asynchronous architecture effectively isolates the vision workload from the control loop. The rare maximum periods (100–185 ms) are attributable to non-real-time operating-system scheduling jitter (Windows + Python), not to systematic blocking by the mechatronic control pipeline.
...
**3. Asynchronous perception-control separation improves deployability.** The asynchronous architecture separates the approximately 20 Hz vision sub-process from the nominal 200 Hz control loop, preventing the ~50 ms YOLO inference from occupying the main-thread time slice. Dedicated profiling on the same hardware (Table 1b) showed that the median control period remained at 5.07–5.14 ms and the fraction of cycles exceeding 10 ms stayed at ~6.6% regardless of whether the YOLO sub-process was active, confirming that the non-blocking design operates reliably under realistic task conditions. This perception-control-decoupled mechatronic design pattern lowers the real-time requirements on the vision module, making the system easier to deploy onto existing teleoperation platforms without substantial modification to the control loop.
