# Vision-semantic cross-domain parameter co-synthesis for an integrated haptic teleoperation mechatronic system
...
**Originality**
The originality lies in a cross-domain mechatronic parameter co-synthesis framework for heterogeneous-object haptic teleoperation. The framework temporally decouples low-frequency semantic perception from the nominal high-frequency teleoperation loop, while functionally coupling object semantics with slave-side impedance, operator-facing haptic-interface settings and gripper execution before contact. The proposed three-level mapping and strategy-locking mechanism provide a deployable pre-contact parameter initialization paradigm that distinguishes perception, manual selection, impedance-only scheduling and full multi-channel coordination at the system level.
...
**Control layer**: The nominal 200 Hz main control loop acts as a supervisory teleoperation loop. It executes: read Omega.7 pose increment → apply position scale and coordinate mapping → update slave desired pose → update the Franka Cartesian impedance command → read slave-side external force estimate → render master-side basic haptic feedback. The nominal control period is 5 ms, and the loop is not directly blocked by vision inference. Low-level joint control, internal safety functions and actuator-level servo execution remain handled by the Franka controller; the present implementation does not claim direct low-level torque control from the Python supervisory loop.
...
To make the small-participant design more transparent, operator-level differences are reported explicitly rather than hidden inside pooled trials. The aggregated C–E completion-time differences were −1.66 s for P01, −2.57 s for P02 and −1.15 s for P03. These three operators differ in absolute task speed and likely manipulation style, but all show the same C-faster-than-E direction. Thus, the present evidence is better interpreted as **within-operator robustness across the tested operators**, not as population-level human-factor generalization.
...
**1. Perception should not only inform the operator; it should initialize low-level interface parameters.** The present results suggest that upgrading visual semantic information from "operator cue" to "mechatronic parameter feedforward" can improve system-level performance within the tested platform without increasing the operator's explicit parameter-selection burden. The visual perception module should not merely be information on a display; it should be a link in the mechatronic parameter initialization chain.
...
**3. Asynchronous perception-control separation improves deployability.** The asynchronous architecture separates the approximately 20 Hz vision sub-process from the nominal 200 Hz teleoperation loop. Because visual inference is executed outside the control-loop critical path and updates the strategy only at the initial locking event, the perception module does not need to satisfy the same timing requirement as the master–slave control update. This perception-control-decoupled mechatronic design pattern lowers the real-time burden on the vision module and makes the system easier to deploy onto existing teleoperation platforms without requiring the vision module to run at the control-loop frequency.
...
8. Parameters were determined by physics-constrained heuristic design, safety ranges and pre-experiments. This paper demonstrates the effectiveness of discrete semantic strategies in the current mechatronic task, not the global optimality of the parameters. A formal sensitivity analysis around each parameter level is still needed to quantify how performance changes under ±10% or larger perturbations.
