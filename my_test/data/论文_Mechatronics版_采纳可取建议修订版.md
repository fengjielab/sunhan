...
The values in Table 2 are not derived from object-level modal identification, calibrated contact-force limits or global optimization. Instead, they are selected within the stable, comfortable and executable operating ranges of the robot arm, haptic interface and gripper. The purpose of the table is to define a reproducible mechatronic design space for pre-contact parameter initialization: lower values bias the system toward compliance and gentle grasp establishment, whereas higher values bias the system toward positioning stability, stronger interface cues and faster grasp execution.
...
When vision is not locked or the detection result is unmappable, the system uses the default balanced-strategy parameters. Intra-task strategy locking prevents frequent parameter jumps due to vision jitter. The arm's built-in collision detection, program-exit zero-force command, unified initial pose, and manual emergency stop jointly constitute the basic safety measures. If an object is misclassified as another valid class, the system may select a suboptimal but still bounded parameter set from the same predefined table. This could degrade task performance, but it does not introduce unbounded online parameter changes because all scheduled values remain inside the frozen hardware-constrained design space and the strategy remains locked during the task. This paper focuses on pre-contact parameter initialization and does not discuss online post-contact optimal tuning.
...
9. This paper reports a non-blocking perception-control architecture with a nominal 200 Hz teleoperation loop, but does not provide a full hard real-time profiling study. Cycle-level jitter analysis under real-time operating-system conditions will be considered in future implementation work.
