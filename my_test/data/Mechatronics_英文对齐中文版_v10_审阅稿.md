...
- **伦理审批：** **[投稿阻断项]** 本研究涉及三名成年志愿者。投稿前，应以相关机构委员会或办公室出具的书面认定替换本句，写明机构、决定类型（批准、豁免或无需正式审查）、日期、参考编号（如有）和实际依据。所有参与者均知悉实验程序并在参与前书面知情同意；未收集可识别个人信息，数据已匿名化并仅用于学术研究。
- **图像完整性：** 图1为实际实验平台照片。未使用生成式 AI 增加、删除、替换或重新定位实验元素。图2–3由作者使用 Python/Matplotlib 重绘，图4–9由实验数据生成。作者核验了最终图中的逻辑、文字和数据映射。
- **生成式 AI 与 AI 辅助技术声明：** 稿件准备期间，作者使用 OpenAI Codex 辅助语言编辑、一致性检查及 Python/Matplotlib 代码草拟。作者随后审阅并编辑所有输出，核验技术陈述和数据映射，并对稿件内容承担全部责任。
- **基金：** 本研究未获得外部基金资助。
- **利益冲突：** 作者声明不存在利益冲突。
