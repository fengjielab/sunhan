...
class ExperimentTimeline:
    ...
    def mark_repeatable(self, name: str, now: Optional[float] = None, **details) -> None:
        """Append an event such as every visual detection without deduplicating it."""
        with self._event_lock:
            now = time.perf_counter() if now is None else now
            event = {
                "event": name,
                "system_time": now - self.start_perf,
                "phase": self.phase,
            }
            event.update(details)
            self.events.append(event)
