...
The selected levels were engineering operating points chosen after qualitative screening, not optimized values or formal physical safety limits. Their intended directions describe the controller or API behavior used in this platform and task; they should not be interpreted as independently identified material properties or fingertip-force measurements.
\small
...
\midrule
 ...
 Master haptic: $d$ & Lower $d$ $\rightarrow$ greater sensitivity to small forces; higher $d$ $\rightarrow$ greater suppression of small signals. & Master-side haptic jitter and noise. & 0.3 / 0.4 / 0.5 N \\
 Gripper: $v_g$ & Lower $v_g$ $\rightarrow$ slower and lower-impact closing; higher $v_g$ $\rightarrow$ faster grasp establishment. & Franka Hand execution range. & 0.02 / 0.05 / 0.10 m/s \\
 Gripper: $F_g$ & Lower requested $F_g$ $\rightarrow$ gentler grasp; higher $F_g$ $\rightarrow$ larger requested grasp force. & Franka Hand force setting. & 8 / 15 / 20 N \\
\bottomrule
\end{longtable}
\normalsize
...
\end{tabular}
\end{center}
\noindent Values are mean completion time in seconds; bold indicates the lowest mean in each row.
\normalsize

\subsection*{Table S6. Evidence-boundary overview}
\small
\renewcommand{\arraystretch}{1.18}
\begin{tabular}{p{0.28\textwidth}p{0.39\textwidth}p{0.25\textwidth}}
\toprule
Claim & Direct evidence in the present study & Supported interpretation \\
\midrule
Complete multi-channel configuration was associated with a lower mean task completion time than impedance-only reconfiguration. & The paired C--E mean difference was 1.79 s (matched-block bootstrap 95\% CI, 1.10--2.51 s); 22 of 27 matched blocks favored mode C. & Supported as a system-level, within-sample result under the tested conditions. \\
\addlinespace[2pt]
...
\addlinespace[2pt]
The master-side haptic and gripper-execution channels each contribute independently, or their combination is synergistic. & Modes C and E changed the master-side haptic and gripper-execution parameter channels together; no factorial channel-ablation design was included. & Not tested. \\
\addlinespace[2pt]
...
\small\noindent\textbf{Figure S1.} Confusion matrix for the separate closed-set controlled visual test (180 images; 30 images per class). All observations lie on the diagonal under the fixed viewpoint, background, and lighting conditions.
\end{figure}
\end{document}
