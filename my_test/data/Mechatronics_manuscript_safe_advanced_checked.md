# Vision-semantic cross-domain parameter co-synthesis for integrated haptic teleoperation with object-specific mechatronic reconfiguration

## Structured Abstract

**Purpose**
This paper addresses the engineering challenge that fixed teleoperation parameters are difficult to tune for heterogeneous objects with different levels of fragility, stiffness, and grasping requirements. In such tasks, manual parameter tuning is non-intuitive because impedance, haptic-interface feedback, dead-zone thresholds, gripper speed, and grasping force jointly affect the operator-facing hand-feel and the execution behavior of the slave robot. We propose a vision-semantic multi-channel parameter scheduling framework that bridges object-level semantics and low-level mechatronic interface parameters by establishing a three-level mapping from perception to operation-oriented strategies and control parameters before contact.

**Design/methodology/approach**
The mechatronic system integrates an asynchronous RGB-D perception thread, a 200 Hz master–slave Cartesian impedance control loop, a haptic-interface parameter pre-setting module, and gripper execution on an Omega.7–Franka Panda–Franka Hand platform. Before contact, the RGB-D perception module identifies the target object and parameterizes the mechatronic interface by assigning one of three operation-oriented policies: fragility-oriented, balanced, and stability-oriented. The proposed scheduler then coordinates slave-side translational/rotational stiffness, damping ratio, master-side haptic-interface gain, force-interface dead zone, gripper closing speed, and grasping force through a discrete, interpretable parameter table. Three operators performed 135 grasping trials involving six objects and five modes: fixed parameters, manual selection, full vision-semantic multi-parameter scheduling, visual information only, and vision-semantic impedance-only scheduling.

**Findings**
On the tested mechatronic platform and with the participating operators, the proposed mode achieved the shortest median completion time (19.57 s [18.41, 20.05] IQR), the highest success rate as a descriptive metric (26/27, 96.3%) and the lowest Raw NASA Task Load Index (NASA-TLX) score (median 48.67 [47.67, 51.83] IQR). Compared with impedance-only scheduling, the proposed mode yielded a mean completion-time difference of -1.79 s (Bootstrap 95% CI [-2.51, -1.10] s; corresponding to a relative reduction of 8.5%), while all three operators and all six tested objects showed the same directional trend. The trajectory-length difference was smaller (-0.024 m, 95% CI [-0.059, 0.014] m), suggesting that the performance gain was more consistent with reduced non-productive pauses and operational corrections than with a clearly shorter geometric path. The Raw NASA-TLX difference was -4.87 points (Bootstrap 95% CI [-5.35, -4.39]).

**Originality**
The originality lies in a cross-domain mechatronic parameter co-synthesis framework for heterogeneous-object haptic teleoperation. The framework temporally decouples low-frequency semantic perception from the nominal high-frequency teleoperation loop, while functionally coupling object semantics with slave-side impedance, operator-facing haptic-interface settings and gripper execution before contact. The proposed three-level mapping and strategy-locking mechanism provide a deployable pre-contact parameter initialization paradigm for object-specific mechatronic reconfiguration, while distinguishing perception, manual selection, impedance-only scheduling, and full multi-channel coordination at the system level.

**Keywords:** mechatronic system integration; haptic teleoperation; object semantics; Cartesian impedance control; perception-control integration; human-in-the-loop experiment

---

## 1 Introduction

Teleoperated robots combine human judgment with remote robotic execution and are well suited to flexible manufacturing, hazardous-environment operations, service robotics, and unstructured object manipulation. Haptic teleoperation further conveys remote contact information to the operator through master-side force feedback, improving the operator's perception of grasping, contact, and slip risks. From a mechatronic system design perspective, a practical haptic teleoperation platform must integrate multiple subsystems including mechanical actuation, human–machine interfaces, visual perception, real-time control, and parameter scheduling [1–4].

In real grasping tasks, operators encounter objects with diverse shapes, surfaces, fragility, and gripping requirements. If the system always uses a fixed set of impedance and gripper parameters, it must compromise among compliant contact, response speed, positioning stability, and grasping reliability. This problem is not limited to laboratory tabletop grasping. Similar requirements arise in remote maintenance, hazardous material handling, flexible sorting, unstructured disassembly, and human-supervised robotic operations, where the mechatronic system needs to handle fragile items, slippery objects, rigid tools, lightweight containers, or geometrically irregular objects in partially structured environments. The six objects selected in this study—apple, banana, paper cup, bottle, mouse, and scissors—are not intended to cover a complete industrial object set. Instead, they serve as a **mechatronic benchmark task set** and are selected to generate distinct requirements on compliance, haptic-interface sensitivity, and gripper execution, rather than to represent a complete industrial object taxonomy.

Impedance control provides compliance during contact by prescribing the dynamic relationship among robot displacement deviation, velocity deviation, and interaction force [5]. Fixed-impedance control is simple to implement and easy to deploy, but cannot simultaneously accommodate fragile, lightweight, and rigid objects during heterogeneous grasping. Variable impedance control can adjust stiffness and damping online according to contact force, trajectory error, task phase, or human motion states [6–9]. However, online variable impedance typically depends on continuous state estimation, post-contact feedback, and stability constraints. For teleoperation tasks where object categories are identifiable and task procedures are relatively fixed but still require the operator to perform fine grasping and placement, pre-contact object semantics can serve as a low-cost, interpretable task prior.

Prior studies on visual impedance, shared control, and haptic guidance have shown that visual information, task state, or operator intent can improve remote-operation efficiency and interaction experience [10–13]. However, in heterogeneous-object haptic teleoperation, object semantics affect not only slave-side arm compliance but also the haptic-interface feedback intensity, dead zone, gripper closing speed, and grasping force that operators expect. When visual information is displayed without changing the system dynamics, the operator must still compensate manually. When only impedance is adjusted while the default force-feedback and gripper parameters remain unchanged, the grasping and transport phases may still be constrained by the gripper and haptic-feedback channels. Therefore, the question addressed in this paper is: In real haptic teleoperation grasping, does pre-contact, vision-semantic-driven multi-channel mechatronic parameter coordination outperform fixed parameters, manual selection, visual cues, and impedance-only adjustment?

This paper proposes a vision-semantic-driven multi-channel parameter scheduling method from a mechatronic system integration perspective. The system uses the target object semantics to parameterize a bounded mechatronic interface: it assigns one of three operation-oriented strategies and invokes a discrete, interpretable parameter table before contact, thereby simultaneously configuring slave-side impedance, master-side haptic-interface parameters, and gripper execution parameters. The contributions are as follows:

1. **Mechatronic system architecture integration.** RGB-D perception, a nominal 200 Hz supervisory control loop, Cartesian impedance, haptic-interface presetting, and gripper execution are unified within an asynchronous perception–control-decoupled mechatronic architecture, with seven explicitly defined subsystem layers and their coordination relationships.
2. **Three-level mapping: vision semantics–operation strategy–control parameters.** Object category is transformed into fragility-oriented, balanced, and stability-oriented strategies, allowing visual information to enter the mechatronic teleoperation system as a pre-contact control prior for object-specific parameter reconfiguration.
3. **Cross-domain mechatronic parameter co-synthesis.** Beyond adjusting slave-side translational/rotational stiffness and damping ratio, the method simultaneously presets master-side haptic-interface gain, force-interface dead zone, gripper closing speed, and grasping force, thereby coupling perception, operator interface, and mechanical execution as a single pre-contact parameter group.
...
Unlike studies that rely on online post-contact adaptation or continuous visual servoing, the novelty of this paper lies in a cross-domain pre-contact co-synthesis paradigm: object semantics are converted into a bounded mechatronic parameter group before contact, while the low-frequency perception process remains temporally decoupled from the nominal teleoperation loop. The haptic-interface gain and dead zone are used as operator-facing interface presets in the strategy table, and their independent closed-loop force-feedback contribution is deliberately not claimed. The core positioning of this paper is therefore an interpretable, computationally efficient, and readily deployable mechatronic-system parameter initialization method for heterogeneous-object teleoperation.
...
The mechatronic teleoperation platform consists of seven subsystem layers that work in coordination. The information flow and system architecture are illustrated in Figs. 1 and 2.
...
**Mechanical execution layer**: The Franka Panda 7-DOF robot arm and Franka Hand gripper constitute the execution side. The Panda responds to desired pose and impedance commands via Cartesian impedance control; the Franka Hand executes open/close actions at a specified speed and grasping force.

**Human–machine interface layer**: The Omega.7 7-DOF force-feedback master device captures the operator's displacement and gripper input and renders slave-side interaction forces through the basic haptic interface. The operator controls gripper closure via the gripper opening; the system sets the gripper speed and force upper bounds according to the parameter table of the current strategy.

**Perception layer**: An Intel RealSense D435i RGB-D camera captures RGB-D images at approximately 20 Hz. The YOLO11n object detection model runs in an independent subprocess (decoupled from the Python GIL), with a per-frame processing time of approximately 50 ms. Detection results are transmitted back to the main control thread via `multiprocessing.Queue(maxsize=2)`, with shared state protected by `threading.Lock`.

**Control layer**: The nominal 200 Hz main control loop acts as a supervisory teleoperation loop. It performs the following sequence: read Omega.7 pose increment → apply position scale and coordinate mapping → update slave desired pose → update the Franka Cartesian impedance command → read slave-side external force estimate → render master-side basic haptic feedback. The nominal control period is 5 ms, and the loop is not directly blocked by vision inference. Low-level joint control, internal safety functions, and actuator-level servo execution remain handled by the Franka controller; the present implementation does not claim that low-level torque control is directly implemented by the Python supervisory loop.

**Vision thread**: The vision subprocess asynchronously receives RGB frames (`mp.Queue(maxsize=1)` retains only the latest frame to avoid latency accumulation), performs object detection, and returns class labels and confidence. The vision thread is fully decoupled from the control thread; the maximum vision inference time (~50 ms) does not directly occupy the main-thread time slice of the nominal 200 Hz control loop.

**Parameter scheduling layer**: After vision results enter the scheduling layer, the object-class-to-strategy mapping retrieves the seven-parameter set (\(K_t, K_r, \zeta, K_f, d, v_g, F_g\)) from the preset parameter table and writes the values into the control-loop shared variables via atomic updates. Parameter updates are triggered only at the strategy-lock event after the first valid detection; thereafter, parameters remain unchanged for the entire task episode.
...
| Slave control | 200 Hz | \(\mathbf{x}_d, \mathbf{K}, \mathbf{D}\) | Desired pose / impedance command to Panda controller | No |
| Vision detection (YOLO11n) | ~20 Hz (50 ms/frame) | RGB-D image | object class, confidence | No (subprocess) |
| Strategy scheduler | Event-based (on first valid detection) | class, confidence | \(\Theta(c)\) parameter set | No |
| Haptic rendering | 200 Hz | \(\mathbf{F}_{\mathrm{ext}}, K_f, d\) | Omega.7 force vector | No |
| Gripper command | Event-based (gripper button) | gripper input, \(v_g, F_g\) | Franka Hand grasp/goal | No |

Vision detection and strategy scheduling are both event-driven and do not block the main control loop. The vision subprocess runs as an independent Python process, using `multiprocessing.Queue` in a producer-consumer pattern to pass detection results. The frame queue length is limited to 1 (retaining only the latest frame), and the result queue length is limited to 2. At the start of each 5 ms period, the main control thread non-blockingly reads the result queue; if the queue is empty, the previously locked strategy is retained; if vision has not yet locked a strategy, the default balanced parameters are maintained. Strategy locking occurs before the approach phase begins: once the main control thread detects a valid class with confidence ≥ 0.25, it immediately locks the strategy and sets all seven parameters atomically; thereafter, parameters remain unchanged for the entire task episode.

The implementation uses a nominal 200 Hz master–slave control loop and an asynchronous RGB-D perception process. Vision inference is executed outside the control-loop critical path in a separate process and communicates with the control loop through bounded queues. The control loop reads detection results in a non-blocking manner and retains the previously locked strategy when no new result is available. Therefore, the vision module is not required to satisfy the same timing constraint as the master–slave control update, and visual inference is not placed in the critical path of the teleoperation loop. By executing visual inference outside the control-loop critical path, the asynchronous architecture reduces direct latency coupling between semantic recognition and the nominal 200 Hz haptic teleoperation loop, rather than claiming hard real-time control performance.

**Fig. 2(b).** Timing diagram of the asynchronous perception-control architecture. The diagram should show the nominal 5 ms teleoperation control cycles, the approximately 50 ms vision inference cycle, the first valid detection event, strategy locking, approach, grasp, transport, and release phases. Strategy locking is designed to occur before the approach phase; therefore, the scheduled parameters are used as pre-contact initialization rather than as reactive switching at the instant of contact. If the detection result is delayed, absent, or below the confidence threshold, the controller retains the balanced default strategy until a valid lock event occurs.
...
Parameter selection follows the engineering logic of "object operational risk → control response → hardware constraint." Table 3 presents the design space for each parameter: the physical meaning of low and high values, and the hardware/safety boundaries that constrain the values used in this study. In this paper, the parameter table is treated as a **physics-constrained heuristic design**, not as an unconstrained empirical lookup table. The design is bounded by four classes of physical constraints: robot-side impedance stability and command limits, master-side haptic comfort and force saturation, gripper speed/force execution limits, and task-level safety requirements for fragile or slippery objects.
...
The values in Table 2 are not derived from object-level modal identification, calibrated contact-force limits or global optimization. Instead, they are selected within the stable, comfortable, and executable operating ranges of the robot arm, haptic interface and gripper. The purpose of the table is to define a reproducible mechatronic design space for pre-contact parameter initialization: lower values bias the system toward compliance and gentle grasp establishment, whereas higher values bias the system toward positioning stability, stronger interface cues and faster grasp execution. The three levels for each channel should therefore be interpreted as bounded design points inside a verified operating envelope, rather than as globally optimal values. A full sensitivity analysis around each channel is outside the current experiment; the present study tests whether the resulting bounded parameter group is useful as a system-level pre-contact initialization strategy.

The parameter ranges are jointly constrained by the Franka control interface, Omega.7 feedback-interface comfort, Franka Hand execution capability, and the results of pre-experiments. Pre-experiments were performed by two researchers before the formal experiment and covered grasping operations for all three object categories, to exclude parameter combinations that were obviously unsafe, inefficient, or subjectively unacceptable to the operator. **The parameter table was finalized before the formal 135 trials and was not modified after observing the formal experimental results.** Mode B uses the same parameter table, but the operator manually selects the strategy. Therefore, Mode B is defined in this paper as a **manual-selection workflow baseline**, not a pure automatic-vs-manual controller performance comparison. Manual-selection time is included in the total completion time for Mode B; thus, Mode B evaluates the actual workflow including human judgment and switching overhead, rather than serving as a pure controller execution-time baseline.

### 2.8 Algorithm Flow

**Algorithm 1: Vision-semantic multi-channel parameter scheduling for the integrated mechatronic system**

1. Initialize the system, load the balanced-strategy default parameters \(\Theta(\text{balanced})\), start the 200 Hz main control loop and the vision subprocess.
2. The vision subprocess asynchronously reads RGB-D images and performs YOLO11n object detection (does not block the main loop).
...
5. After locking, parameters remain unchanged for the entire task episode, thereby preventing frequent switching due to vision jitter.
...
When vision is not locked or the detection result is unmappable, the system uses the default balanced-strategy parameters. Intra-task strategy locking prevents frequent parameter jumps due to vision jitter. The arm's built-in collision detection, gripper force limitation, program-exit zero-force command, unified initial pose, and manual emergency stop jointly constitute the basic safety measures.

**Semantic misclassification and fail-safe behavior.** If an object is misclassified as another valid class, the system may select a suboptimal but still bounded parameter set from the same predefined table. Such an error could degrade efficiency or object-specific suitability; for example, a fragile object assigned to a stability-oriented strategy would receive unnecessarily stiff and fast settings. However, this failure mode does not introduce unbounded online parameter changes because all scheduled values remain inside the frozen hardware-constrained design space and the strategy remains locked during the task. Low-confidence and unmappable detections fall back to the balanced strategy, while robot collision detection, gripper force limits and operator emergency stop remain available as secondary protection. Therefore, the current architecture should be interpreted as a bounded pre-contact parameter initialization scheme, not as a complete industrial fail-safe system for arbitrary misclassification. This paper focuses on pre-contact parameter initialization and does not discuss online post-contact optimal tuning.
...
- **RQ1:** Does vision-semantic multi-parameter feedforward scheduling outperform fixed parameters, manual selection, and vision-only observation?
...
Three operators (P01–P03, 23–24 years old, all male and right-handed) participated in the main experiment. All three operators had basic teleoperation training experience and completed 10–15 minutes of warm-up trials before each formal session. All operators provided written informed consent. This study does not involve medical intervention and does not collect personally identifiable information.

The experiment includes six objects, forming a **mechatronic benchmark task set** designed to generate distinct requirements on compliance, haptic-interface sensitivity, and gripper execution conditions. The six objects are assigned to fragility-oriented, balanced, and stability-oriented strategies. This classification is used for parameter scheduling in the present experimental task and does not claim to represent a universal physical classification of object material properties.
...
Mode order was partially balanced during the experiment to reduce learning or fatigue bias from a single fixed sequence. Because strict full randomization was not performed and all objects, operators, and order factors were not completely decoupled, this paper does not claim that order effects were fully eliminated; rather, these are conservatively discussed in the limitations section. The complete trial-by-trial execution sequence is provided as supplementary material.
...
The primary endpoint is completion time. Secondary objective endpoints include success rate, master trajectory length, pause count, direction-reversal count, and motion smoothness. Subjective workload is assessed using the unweighted Raw NASA Task Load Index (NASA-TLX), calculated as the arithmetic mean of six dimensions. NASA-TLX scores were collected at the "operator × object strategy × mode" level. The vision module reports class-level recognition accuracy, strategy-trigger accuracy, confidence, and per-frame processing time.

Process-behavior metrics were defined and fixed before formal statistical analysis. A pause is defined as master velocity below 0.005 m/s for a duration of at least 0.30 s, detected in real time by differencing velocity from the raw master trajectory CSV (sampled at approximately 200 Hz).
...
Under controlled viewpoint, background, and illumination, 30 images were collected for each object class, yielding 180 images in total. Class recognition and strategy triggering were successful in 180/180 cases (100%), with mean confidence 0.853 and per-frame wall-clock processing time 50.08 ms. This result indicates that visual triggering was not a major source of error under the present experimental conditions, but does not generalize to occlusion, strong illumination variation, cluttered backgrounds, unknown objects, or untested classes.
...
**Table 4.** Five-mode experimental results: completion time, master trajectory length, success rate, and Raw NASA-TLX. Values are reported as median [IQR] with mean±SD in parentheses.
...
The Friedman test on five-mode completion times indicates a global difference among the five modes (χ²(4)=30.904, p<0.001). Paired Wilcoxon tests with Holm correction show that Mode C has a lower completion time than Modes A, B, D, and E in the paired non-parametric analysis (p < 0.01, effect size r > 0.7). Because trials are nested within three operators, these p values and effect sizes are interpreted as paired evidence within the current mechatronic platform, object set, and participant pool, rather than as general-population statistical conclusions. Raw NASA-TLX is also lowest for Mode C, but subjective workload results are interpreted cautiously given the small sample and non-blinded conditions.
...
The C–E comparison is the most critical **system-level** ablation in this paper. Both modes use vision semantics and impedance adjustment; the difference is that Mode C additionally sets master-side haptic-interface gain, force-interface dead zone, gripper closing speed, and grasping force. This design tests an engineering question: whether adjusting slave-side compliance alone is sufficient to cover heterogeneous-object grasping requirements, or whether the operator-facing interface and gripper execution channels need to be initialized together with impedance. The comparison should not be interpreted as a component-identification experiment. It demonstrates the usefulness of the complete pre-contact parameter group over impedance-only adjustment, but it cannot independently rank the causal contributions of \(K_f\), \(d\), \(v_g\) and \(F_g\).

In the 27 matched blocks, the median completion time for Mode C is 19.57 s and that for Mode E is 20.73 s, with a paired mean difference of −1.79 s (Bootstrap 95% CI [−2.51, −1.10] s), representing a relative reduction of approximately 8.5%. The CI does not cross zero, supporting a consistent paired improvement in completion time for Mode C within the tested platform, operators, and objects. Operator-level aggregated results show that all three operators exhibit a C-faster-than-E direction: P01 18.94 s vs. 20.60 s (−8.1%), P02 19.09 s vs. 21.66 s (−11.8%), P03 19.80 s vs. 20.95 s (−5.5%). All six objects also show a C-faster-than-E direction (reduction range 3.3%–13.2%).
...
Pause counts were computed from the raw master trajectory CSVs (sampled at approximately 200 Hz). A pause is defined as master velocity below 0.005 m/s for a duration of at least 0.30 s. The per-trial median pause count for Mode C is 3 [2, 3.5] IQR (mean: 2.74±1.23), and that for Mode E is 3 [2, 5] IQR (mean: 3.41±1.67). By strategy, fragility-oriented, balanced, and stability-oriented categories all show a direction of fewer pauses in Mode C. This result is consistent with the observation that Mode C has a shorter completion time while the trajectory-length difference is small, supporting the interpretation that the additional benefit of multi-channel coordination is more consistent with reduced non-productive pauses and operational corrections than with path shortening. Future work will introduce phase-level time annotation (e.g., approach, grasp, transport, release) for finer-grained attribution analysis of mechatronic system behavior.
...
Mode C had the lowest failure count (1/27) in the present experiment. This observation is consistent with the interpretation that the full multi-channel parameter strategy may improve grasping stability, but because of the absence of phase-level logs, contact-force measurements, and slip measurements, this paper does not interpret the failure count difference as a directly proven causal mechanism. The present failure records are task-level rather than phase-level; therefore, they cannot determine whether impedance-only failures occurred mainly during grasp establishment, transport, or release. The observed unstable-grasp cases should be treated as qualitative support for the need to coordinate gripper execution with impedance, rather than as phase-specific causal proof.
...
The C-over-E time reduction ranges from 3.3% (bottle) to 13.2% (scissors), with intermediate reductions of 5.5% for banana, 8.1% for apple, 8.7% for mouse, and 11.7% for paper cup. This indicates directional consistency across the six tested objects, but does not extrapolate to untested objects or complex occlusion scenarios.

To make the small-participant design more transparent, operator-level differences are reported explicitly rather than embedded in pooled trial-level results. The aggregated C–E completion-time differences were −1.66 s for P01, −2.57 s for P02 and −1.15 s for P03. These three operators differ in absolute task speed and likely manipulation styles, but all show the same C-faster-than-E direction. Thus, the present evidence is better interpreted as **within-operator robustness across the tested operators**, not as population-level human-factor generalization.

**Fig. 7.** Operator-level C–E variability. The figure should display paired C–E completion-time differences for P01–P03, preferably as operator-facet paired scatter plots or \(\Delta T=T_C-T_E\) distributions, to show whether the direction of improvement is consistent despite inter-subject variability.
...
**1. Perception should not only inform the operator; it should initialize low-level interface parameters.** The present results suggest that upgrading visual semantic information from an "operator cue" to "mechatronic parameter feedforward" can improve system-level performance within the tested platform without increasing the operator's explicit parameter-selection burden. The visual perception module should not merely provide information on a display; it should be a link in the mechatronic parameter initialization chain.

**2. Compliance adaptation alone is incomplete for grasping.** Adjusting only slave-side impedance (Mode E in the C–E ablation) provides compliance adaptation but cannot cover gripper execution speed and grasping force, nor can it adjust the haptic-interface intensity and dead zone perceived by the operator. A complete mechatronic teleoperation grasping system should co-schedule impedance, haptic-interface, and gripper execution as a coupled parameter set.

**3. Asynchronous perception-control separation improves deployability.** The asynchronous architecture separates the approximately 20 Hz vision subprocess from the nominal 200 Hz teleoperation loop. Because visual inference is executed outside the control-loop critical path and updates the strategy only at the initial locking event, the perception module does not need to satisfy the same timing requirement as the master–slave control update, and semantic-recognition latency is prevented from directly occupying the nominal haptic-loop time slice. This perception-control-decoupled mechatronic design pattern lowers the timing burden on the vision module and makes the system easier to deploy onto existing teleoperation platforms without requiring the vision module to run at the control-loop frequency.

### 5.5 Why the Benefit Magnitude Differs Across Objects

All six objects show a C-faster-than-E direction, but the reduction magnitude is not uniform. The smaller reductions for bottle and banana may be attributable to the relative familiarity of their grasping actions, and operators could compensate for default gripper parameters through experience even in Mode E. The larger reductions for paper cup and scissors may be associated with higher grasping risk and pose-stability requirements: the paper cup requires avoidance of deformation and unstable gripping, and the scissors require more explicit positioning and stable transport. This mechanistic interpretation could be further verified through finer-grained data including phase-level durations, re-grasp behavior, and gripper state logs.
...
6. Vision validation was performed under controlled viewpoint, background, and illumination; the 100% accuracy represents only controlled experimental conditions and does not extrapolate to occlusion, cluttered backgrounds, or untested classes. A wrong but valid class could still trigger a suboptimal parameter set, which is a limitation of semantic pre-contact scheduling.
...
8. Parameters were determined by physics-constrained heuristic design, safety ranges and pre-experiments. This paper demonstrates the effectiveness of discrete semantic strategies in the current mechatronic task, rather than the global optimality of the parameters. A formal sensitivity analysis around each parameter level is still needed to quantify how performance changes under ±10% or larger perturbations.
9. The C–E comparison tests the coupled parameter group against impedance-only scheduling, but does not isolate the independent contributions of haptic-interface parameters and gripper execution parameters. Component-wise ablations such as impedance+gripper-only or impedance+haptic-only scheduling are left for future work.
10. This paper reports a non-blocking perception-control architecture with a nominal 200 Hz teleoperation loop, but does not provide a full hard real-time profiling study. Cycle-level jitter analysis under real-time operating-system conditions will be considered in future implementation work.

---

## 6 Conclusion

This paper proposes a vision-semantic multi-channel parameter scheduling method for heterogeneous-object haptic teleoperation grasping from a mechatronic system integration perspective. The method interprets the target object category as one of three operation-oriented strategies—fragility-oriented, balanced, and stability-oriented—and coordinates slave-side impedance, master-side haptic-interface parameters, and gripper execution parameters before contact, forming an asynchronous perception-control-decoupled mechatronic teleoperation system. Real-platform five-mode experiments show that, within the current three-operator, six-object, 135-trial mechatronic benchmark task set, the vision multi-parameter mode achieves the shortest median completion time (19.57 s [18.41, 20.05] IQR), the highest success rate (26/27, 96.3%), and the lowest Raw NASA-TLX (median 48.67 [47.67, 51.83] IQR). Compared with the vision impedance-only mode, the full multi-channel mode yields a mean completion-time difference of −1.79 s (Bootstrap 95% CI [−2.51, −1.10] s), with all three operators and all six objects showing a consistent direction. Pause analysis and bootstrap CIs further suggest that the additional benefit is more consistent with reduced non-productive pauses and operational corrections through mechatronic parameter coordination than with a clearly shorter geometric path.

Overall, this paper provides an interpretable, computationally efficient, and readily deployable mechatronic-system pre-contact parameter initialization method for heterogeneous-object haptic teleoperation without requiring complex online optimization. Its statistical generalizability and external validity still require further verification through more operators, strictly randomized order, object-instance-level recording, phase-level process metrics, and contact-quality metrics. Post-contact external force estimation, force-feedback closed-loop correction, and operator haptic perception validation belong to the scope of subsequent studies and are not independently demonstrated by the data in this paper.
