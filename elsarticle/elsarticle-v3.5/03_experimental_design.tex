...
Table~\ref{tab:3} defines the five experimental modes. Mode definitions and channel-update logic are summarized in the table and its note; the C--E comparison was the primary planned contrast.
...
For each operator and strategy category, three matched blocks were allocated between the two assigned objects in a $2{:}1$ ratio, reversed across operators (Table~\ref{tab:6}). The same object was used for all five mode trials within a block, preserving object-matched comparisons.
