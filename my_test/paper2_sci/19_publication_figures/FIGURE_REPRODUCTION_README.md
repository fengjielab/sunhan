...
```powershell
python my_test/paper2_sci/19_publication_figures/scripts/generate_all_figures.py --root .
```
...
|---|---|---|---|---|
| Fig01 | `fig01_framework.py` | `Fig01_realized_intervention_framework` | `figure_source_data/figure01_source_definition.json` | Generic four-layer framework, interface fidelity classes, provenance branch, and admissible interpretation |
| Fig02 | `fig02_system_lineage.py` | `Fig02_system_and_lineage` | `data_lineage_audit.csv`, `participant_level_metrics.csv` | (A) teleoperation system and internal wrench-estimate paths; (B) exact acquisition lineage, trial-level fidelity, and participant-level inference |
...
| Fig04 | `fig04_participant_outcomes.py` | `Fig04_participant_EA_outcomes` | `participant_level_metrics.csv`, `statistics_summary.csv`; `trial_level_fidelity_metrics.csv` for the 9-trial pre-aggregation QA only | (A) participant-paired excess-force impulse; (B) participant-paired task-start-to-contact time; (C) participant-paired total task time; annotations show frozen E-A estimates and 95% CIs |
| Fig05 | `fig05_contact_trajectories.py` | `Fig05_contact_aligned_trajectories` | `contact_aligned_summary.csv`; `contact_aligned_trajectories.csv` and `trial_level_fidelity_metrics.csv` for hierarchical-aggregation and commanded-`K_t` QA | (A) participant-aggregated contact-aligned threshold-referenced excess force; (B) participant-aggregated logged commanded translational stiffness; bands are pointwise t-based 95% CIs across five participant curves |
...
All human-outcome panels display the five participant-level observations. Trial-level fidelity panels explicitly state that trials are fidelity observations, not independent human samples for outcome inference. No smoothing, compliant-only filtering, manual point entry, or outcome-dependent trial selection is used.
