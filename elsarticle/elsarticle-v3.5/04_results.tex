...
\caption{Confidence scores and per-image processing times in the controlled closed-set visual test ($n=30$ images per object class). Bars indicate mean $\pm$ SD; half violins, points, segments, and open circles indicate distributions, individual observations, IQRs, and medians, respectively. Reference lines indicate the confidence threshold of 0.25 and the overall means for confidence (0.853) and processing time (48.19 ms). The reported processing time does not represent end-to-end task-trigger latency.}
\label{fig:4}
\end{figure*}
...
\end{table*}
Across the five modes, mode C showed the lowest median completion time, the highest observed success rate, the lowest median Raw NASA-TLX score, and the lowest median master-side trajectory length (Table~\ref{tab:8} and Figs.~\ref{fig:5}--\ref{fig:6}). According to the trial-level data in Supplementary Table~S4, the mean completion time was 19.28 $\pm$ 1.30 s in mode C, compared with 21.42 $\pm$ 1.58 s in mode A, 21.01 $\pm$ 1.61 s in mode B, 20.91 $\pm$ 1.10 s in mode D, and 21.07 $\pm$ 1.56 s in mode E. With each comparison mode used as the reference, the corresponding mean reductions in mode C were 10.0\%, 8.2\%, 7.8\%, and 8.5\%, respectively. Consistent with the predefined outcome hierarchy, the inferential analyses focused on task completion time as the primary endpoint.
...
A post hoc sensitivity analysis restricted to the 23 blocks in which both trials were successful yielded a similar estimate (mean paired difference, 1.80 s; 19/23 blocks favoring mode C).
...
At the operator level, the mean paired differences $\Delta T=T_E-T_C$ were 1.66, 2.56, and 1.16 s for P01, P02, and P03, respectively, with mode C being faster in 7/9, 9/9, and 6/9 matched blocks. The leave-one-operator-out mean paired differences were 1.86, 1.41, and 2.11 s, all of which remained positive. Operator-level medians [IQR] are shown in Fig.~\ref{fig:8}.
